...
import { preprocess, shortenAudio } from './webaudioUtils.js';
...
const sendFile = document.querySelector('#file-send');
// processImportAudio()
...
function decodeFile(arrayBuffer) {
    ...
    audioCtx.resume().then(() => {
        audioCtx.decodeAudioData(arrayBuffer).then(async function handleDecodedAudio(audioBuffer) {
            ...
            console.log({
                AFTER_AUDIO_CONTEXT_DECODE : audioBuffer
            })
...
function createFeatureExtractionWorker() {
    featureExtractionWorker = new Worker('./src/webfeatureExtraction.js');
...
function createInferenceWorkers() {
    modelNames.forEach((n) => { 
        inferenceWorkers[n] = new Worker('./src/webinference.js');
        inferenceWorkers[n].postMessage({
            name: n
        });
...
function collectPredictions() {
    ...
    if (inferenceResultPromises.length == modelNames.length) {
        Promise.all(inferenceResultPromises, allPredictions).then((predictions) => {
            ...
            console.log({
                Before_Predictions : finshedPredictions
            })

            Object.assign(allPredictions, ...predictions);
            finshedPredictions = {...allPredictions, ...finshedPredictions};
